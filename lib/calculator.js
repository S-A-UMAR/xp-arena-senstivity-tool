const Calculator = {
    ...
    compute(state, globalOffset = 1.0) {
        ...
        // 1. FREE FIRE CORE SENSITIVITY CALCULATION (Scale 0-200)
        // Budget tier needs higher sensitivity to compensate for screen delay. High tier needs less.
        let baseGeneral = tier === 'budget' ? 150 : (tier === 'mid' ? 135 : 120);

        // Adjust based on RAM directly
        if (ram <= 4) baseGeneral += 8;
        if (ram >= 12) baseGeneral -= 8;

        // Playstyle adjustment
        if (playstyle === 'fast') baseGeneral += 15;
        if (playstyle === 'slow') baseGeneral -= 15;

        // Neural AI Offset
        baseGeneral += (scale - 5) * 3;

        // --- NEW: DIAGNOSTIC REACTION OFFSET ---
        if (state.diagnosticData) {
            const { avg_reaction_ms, precision_score } = state.diagnosticData;
            // Extremely fast reaction (< 200ms) -> Lower sensitivity for precision
            // Slower reaction (> 300ms) -> higher sensitivity for compensation
            const reactionDeviation = (avg_reaction_ms - 250) / 10; // Normalized around 250ms
            baseGeneral += reactionDeviation * 1.2;
            
            // Precision bonus: high precision (80+) reduces jitter
            if (precision_score > 80) baseGeneral -= 3;
        }

        // Global Offset
        baseGeneral *= parseFloat(globalOffset);

        // Clamping system (Free Fire max is 200)
        const clamp = (val, min = 10, max = 200) => {
        ...
        // Playstyle-aware spread: slow = tighter (more precise), fast = wider (more aggressive)
        const playstyleSpread = playstyle === 'slow' ? 3 : (playstyle === 'fast' ? 6 : 4);

        const range = (val, spread = playstyleSpread, absoluteMax = 200) => {
            const v = Math.round(val);
        ...
        if (brand.includes('apple') || brand.includes('ios') || brand.includes('iphone')) {
            ...
            // Reaction-based DPI tweak
            if (state.diagnosticData?.avg_reaction_ms < 220) baseDpi -= 20; // Pro snap precision
        ...
        const results = {
            ...
            general: range(baseGeneral),
            redDot: range(redDot),
            scope2x: range(scope2x),
            scope4x: range(scope4x),
            sniperScope: range(sniper),
            freeLook: range(freeLook, Math.max(5, playstyleSpread), 200),
            ads: range(clamp(baseGeneral * 0.93)),
            ...
            diagnostic_sync: state.diagnosticData ? 'TRUE' : 'FALSE',

            // Raw precision for Phase 6 "Precise Mode"
            _raw: {
                general: baseGeneral,
                redDot: redDot,
                scope2x: scope2x,
                ...
                sniperScope: sniper,
                freeLook: freeLook,
                ads: clamp(baseGeneral * 0.93),
                dpi: Math.round(state.diagnosticData?.avg_reaction_ms < 220 ? 411 : 440), // Example logic for raw
                fireButton: btnBase
            }
        };
        ...
        if (state.manualSens && !isNaN(parseFloat(state.manualSens))) {
            ...
            results.general = range(manualVal, 4, 200);
            const redDotManual = clamp(manualVal * 0.98);
            results.redDot = range(redDotManual, 4, 200);
            const scope2xManual = clamp(manualVal * 0.92);
            results.scope2x = range(scope2xManual, 4, 200);
            const scope4xManual = clamp(manualVal * 0.88);
            results.scope4x = range(scope4xManual, 4, 200);
            const sniperManual = clamp(manualVal * 0.50);
            results.sniperScope = range(sniperManual, 4, 200);
            const freeLookManual = clamp(manualVal * 0.80);
            results.freeLook = range(freeLookManual, 5, 200);
            results.isManual = true;
            // Update raw to use manual values
            results._raw.general = manualVal;
            results._raw.redDot = redDotManual;
            results._raw.scope2x = scope2xManual;
            results._raw.scope4x = scope4xManual;
            results._raw.sniperScope = sniperManual;
            results._raw.freeLook = freeLookManual;
        }

...
anced: 1.0, fast: 1.15 };
    ...
    document.addEventListener('DOMContentLoaded', async () => {
      console.info('ADMIN_BOOT: Initializing control system...');
      try {
        ...
        if (!probe.ok) throw new Error(`ADMIN_SESSION_INACTIVE_${probe.status}`);
        console.info('SESSION_VALID: Transitioning to panel');
        unlockAdminPanel();
      } catch (err) {
        console.info('GATE_LOCKED:', err.message);
        showAdminGate();
      }
    });
  </script>

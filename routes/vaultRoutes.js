...
async function createShareToken(entryCode) {
    ...
    return jwt.sign(
        { type: 'share', sid: shareId, code: entryCode },
        await getJwtSecret(),
        { expiresIn: '14d' }
    );
}
...
async function getCodeRecordFromShareToken(shareToken) {
    ...
    if (payload?.type !== 'share' || !payload?.code || !payload?.sid) {
        throw new Error('INVALID_SHARE_TOKEN');
    }
    const shareRecord = await db.get(
        'SELECT share_id, revoked_at, expires_at FROM share_tokens WHERE share_id = ? LIMIT 1',
        [payload.sid]
    );
